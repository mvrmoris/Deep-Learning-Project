...
def compute_and_plot_kendall(df: pd.DataFrame) -> dict | None:
    """
    Calcola Kendall's tau tra accuracy proxy e ground truth
    e genera il grafico dei ranking.
    """
    df_clean = df.dropna(
        subset=["Accuracy", "GT_Accuracy"]
    ).copy()
    ...
    kendall_tau, p_value = stats.kendalltau(
        df_clean["Accuracy"],
        df_clean["GT_Accuracy"]
    )

    print("\n" + "=" * 60)
    print("  KENDALL CORRELATION: SUPERNET PROXY vs NB201 GROUND TRUTH")
    print("=" * 60)
    print(f"  Kendall τ : {kendall_tau:+.4f}  (p = {p_value:.4f})")
    print(f"  Campioni  : {n} architetture")
    print("=" * 60)

    df_clean["rank_supernet"] = df_clean["Accuracy"].rank(
        ascending=False
    )
    df_clean["rank_gt"] = df_clean["GT_Accuracy"].rank(
        ascending=False
    )
    ...
    limit = max(
        df_clean["rank_gt"].max(),
        df_clean["rank_supernet"].max()
    )
    ...
    plt.xlabel("Ground-truth rank (NAS-Bench-201)", fontsize=11)
    plt.ylabel("Supernet proxy rank", fontsize=11)
    plt.title(
        f"Architecture Rank Correlation\n"
        f"Kendall $\\tau$ = {kendall_tau:.3f}",
        fontsize=12,
        fontweight="bold"
    )
