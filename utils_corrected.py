...
def generate_archs(
    ...
    n_samples = min(N, len(architectures))
    generator = torch.Generator().manual_seed(seed)
    indices = torch.randperm(len(architectures), generator=generator)[:n_samples].tolist()
    return [architectures[i] for i in indices]
...
def decoded_x_to_nas201_arch(x_decoded):
    """Convert decoder scores shaped as [4, 4, 5] into a NAS-Bench-201 string."""
    scores = to_numpy(x_decoded).reshape(4, 4, 5)
    nodes = []

    for dst in range(1, 4):
        edges = []
        for src in range(dst):
            op_idx = int(np.argmax(scores[src, dst, :]))
